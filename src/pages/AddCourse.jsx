...
const AddCourse = ({ isEdit }) => {
	...
	const fetchCourses = async () => {
		try {
			...
			setOptions(response.data.courses); // Assuming the response data is an array
		} catch (error) {
	...
	const validateForm = () => {
		...
		if (form.preRequisites.length === 0)
			formErrors.preRequisites = "At least one prerequisite is required";
		if (form.instructor.length === 0)
			formErrors.instructor = "At least one instructor is required";
		return formErrors;
	};
	...
	const handleSubmit = async (event) => {
		...
		try {
			const courseData = {
				...form,
				preRequisites: form.preRequisites.map((pr) => pr.id),
				instructor: form.instructor.map((instr) => instr.id),
				type: "course",
			...
			const response = isEdit
				? await axios.put(
						`${process.env.API_URL}/course/updateCourse/${form.id}`,
						courseData
				  ) // Replace with your API endpoint
				: await axios.post(
						`${process.env.API_URL}/course/createCourse`,
						courseData
				  ); // Replace with your API endpoint
			...
			if (!isEdit) {
				...
				setTimeout(() => {
					navigate("/allcourse"); // Redirect to the courses list page or another appropriate page
				}, 1500);
			}
			setErrors({});
			setApiError("");
	...
	const handleCourseSelection = async (course) => {
		...
		let preRequisites = [];
		try {
			const resInst = await axios.get(
				`${process.env.API_URL}/course/getCourseInstructor/${course.id}`
			);
			const resPre = await axios.get(
				`${process.env.API_URL}/course/getCoursePrerequisite/${course.id}`
			);
			instructor = resInst.data.instructor;
			preRequisites = resPre.data.prerequisiteCourse;
		} catch (error) {
			console.error("Error fetching course data:", error);
			setApiError("Failed to fetch course data.");
		}
			...
			<form onSubmit={handleSubmit} className="px-10">
				...
				<div className="flex mt-5 mr-20">
					...
					<div>
						...
						/>
						{errors.credit && (
							<div className="text-red-500">{errors.credit}</div>
						)}
					</div>
					<div>
						<p className="text-[#2A467C] text-opacity-70 font-bold mb-1">
				...
				<div>
					...
					<Select
						isMulti
						name="preRequisites"
						options={filteredCourses.map((option) => ({
							value: option,
							label: `${option.name} - ${option.courseCode}`,
						}))}
						className="basic-multi-select"
						classNamePrefix="select"
						onChange={handleChange}
						value={form.preRequisites.map((pr) => ({
							value: pr,
							label: `${pr.name} - ${pr.courseCode}`,
						}))}
					/>
				...
				<div>
					...
					<Select
						...
						classNamePrefix="select"
						onChange={handleChange}
						value={form.instructor.map((instr) => ({
							value: instr,
							label: `${instr.name} - ${instr.title}`,
						}))}
					/>
