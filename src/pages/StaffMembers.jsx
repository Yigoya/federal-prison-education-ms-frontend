...
function StaffMembers() {
  ...
  const handleSearch = (e) => {
    ...
    const filteredRows = rows.filter(row =>
      row.firstName.toLowerCase().includes(searchTerm) || row.lastName.toLowerCase().includes(searchTerm)
    ).slice(0, 10);

    setActiveSearch(filteredRows);
  };
  return (
    ...
    <div className='flex mt-5'>
      ...
      <form className='w-[440px] relative m-5'>
        <div className='relative'>
          <input type='search' placeholder='Search by Id or name' className='w-full p-4 rounded-full bg-slate-150' onChange={(e) => handleSearch(e)}/>
...

        {
          activeSearch.length > 0 &&(
            <div className='absolute top-20 p-4 rounded-xl left-1/2 -translate-x-1/2 flex flex-col gap-2'>
              {
                activeSearch.map((row, index) => (
                  <span key={index}>{`${row.firstName} ${row.lastName}`}</span>
                ))
              }
            </div>
          )
